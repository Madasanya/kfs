...
{
    md_history_clear(history_buffer);
}
...
{
    int16_t pos_helper = md_history_get_last_command_index(history_buffer) - (SCREEN_HEIGHT - 1);

    if (pos_helper < 0)
    {
        for (uint32_t i = HISTORY_HEIGHT + pos_helper; i <= HISTORY_HEIGHT - 1; i++)
        {
            md_put_str(md_history_get_entry(history_buffer, i));
        }
        for (uint32_t i = 0; i <= md_history_get_last_command_index(history_buffer); i++)
        {
            md_put_str(md_history_get_entry(history_buffer, i));
        }
    }
    ...
    {
        for (uint32_t i = pos_helper; i <= md_history_get_last_command_index(history_buffer); i++)
        {
            md_put_str(md_history_get_entry(history_buffer, i));
        }
    }
